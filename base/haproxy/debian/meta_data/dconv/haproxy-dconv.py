...
def convert(infile, outfile, base=''):
    ...
    print (f"Importing {infile}...", file=sys.stderr)
    ...
    while i < nblines:
        ...
        if (line == "Summary" or re.match("^[0-9].*", line)) and (len(next) > 0) and (next[0] == '-') \
            ...
            if len(line) > 80:
                print (f"Line {i+1} exceeds 80 columns", file=sys.stderr)
    ...
    for section in sections:
        ...
        print (f"Parsing chapter {title}...", file=sys.stderr)
            ...
            if not title:
                ...
                if re.match("^-+$", pctxt.get_line().strip()):
                    ...
                    if HAPROXY_GIT_VERSION:
                        pctxt.context['headers']['version'] = 'version ' + str(HAPROXY_GIT_VERSION)
    ...
    for keyword in keywords:
        ...
        if len(keyword_chapters) > 1:
            print (f"Multi section keyword : {keyword} in chapters {list(keyword_chapters)}", file=sys.stderr)
    ...
    print (f"Exporting to {outfile}...", file=sys.stderr)
